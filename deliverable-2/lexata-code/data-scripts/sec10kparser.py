...
#   - run this script on different companies and check every single paragragh header combination
#   - need to integrate this new output format into the lexata_ds.py (or change output format to match)
#   - needs to output as a json. convert risk_factors to a dictionary for output.
    ...
    for paragraph in parsed_text:
        ...
        if (not paragraph[0].islower()) and (paragraph in ' '.join(bold_underlined_text)):     # heading
            ...
            if temp_risk_factors == []:
                ...
                temp_risk_factors[1] += paragraph
            else:   # already a paragraph in this risk factor
                temp_risk_factors[1] += '\n' + paragraph
